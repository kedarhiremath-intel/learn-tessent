...
if {[catch {set TEAMAKER_DFT_FLOW_HOME [getenv TEAMAKER_DFT_FLOW_HOME]} errmsg]} { 
   puts "***E: TEAMAKER_DFT_FLOW_HOME not set.  Set this variable in order to continue\n"
   exit -1  
} else {
   puts "***I: TEAMAKER_DFT_FLOW_HOME is set to $TEAMAKER_DFT_FLOW_HOME.\n"
}

if {[catch {set TEAMAKER_DFT_WORK_ROOT_PATH [getenv TEAMAKER_DFT_WORK_ROOT_PATH]} errmsg]} { 
   puts "***E: TEAMAKER_DFT_WORK_ROOT_PATH not set.  Set this variable in order to continue\n"
   exit -1  
} else {
   puts "***I: TEAMAKER_DFT_WORK_ROOT_PATH is set to $TEAMAKER_DFT_WORK_ROOT_PATH.\n"
}

if {[catch {set TEAMAKER_DFT_TECHNOLOGY [getenv TEAMAKER_DFT_TECHNOLOGY ]} errmsg]} { 
   puts "***F: TEAMAKER_DFT_TECHNOLOGY not set.  Set this variable in order to continue\n"
   exit -1
} else {
   puts "***I: TEAMAKER_DFT_TECHNOLOGY is set to $TEAMAKER_DFT_TECHNOLOGY.\n"

   source $TEAMAKER_DFT_FLOW_HOME/dft_common_files/proc_confirm_valid_technology.tcl 
   if { [proc_confirm_valid_technology "$TEAMAKER_DFT_TECHNOLOGY"] == false } {
      exit -1
   }
}
...
define_design_lib WORK -pat ./WORK
...
set componentName intest_edt
set edtOutputBusWidth $edtChannelsOut($block)
