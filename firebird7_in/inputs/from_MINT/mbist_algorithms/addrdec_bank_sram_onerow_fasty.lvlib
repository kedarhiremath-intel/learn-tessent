...
// Address register A loops through all columns of one row
// Address register B shifts through number of bank address bits

Algorithm (IntelLVAddrDecodeBankOneRowFasty){
...
// Address A to keep a track of the TARGET (inner loop)
   AddressRegister (A) {
    ZCarryIn  : X1CarryOut; //X0 X1 -> row; Y0 Y1 -> col, Z -> block
    X1CarryIn : None;
    Y1CarryIn : None;
   }
...
// Address B to keep a track of the BASE (outer loop) 
   AddressRegister (B) {
    ...
    X1CarryIn : None;
    LoadRowAddress : 1'b0;
    LoadColumnAddress : 1'b0;
    LoadBankAddress : 1'b1;
   }
  }
  DataGenerator { 
...
// LoadCounterA_StartCount: 1;
  LoadCounterA_EndCount: NumBankAddressBits;
 }
