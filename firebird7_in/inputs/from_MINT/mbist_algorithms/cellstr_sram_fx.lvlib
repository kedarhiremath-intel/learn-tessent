...
// ^(wI2, neighbors1(wD3), rI4, neighbors2(wD5), rI6, rI7, wD8)
  ...
  // Set B_addr pointer to (X,Y+1)
  Instruction (NOP_1) {  // (X,Y+1): Right 
   OperationSelect  : NoOperation;
...
// neighbors1[(x:y+1), (x-1:y), (x-1:y), (x:y+1)];
  ...
  // (x-1:y),(x-1:y) : TOP
  // Using the WriteData (2 write cycles) for writing to the same location
  ...
  // rI4 (read base)
  // Bring B_addr pointer back to BASE (addr_A)
  Instruction (M1_R1) {
  ...
  // Inc B_addr to go to DOWN location
...
// ^(wD10, neighbors1(wD11), rD12, neighbors2(wD13), rD14, rD15, wD16)
  ...
  // Set B_addr pointer to (x,y+1): right
  Instruction (NOP_4) {   
   OperationSelect  : NoOperation;
   AddressSelectCmd : Select_B;
   Y1AddressCmd : Increment;
