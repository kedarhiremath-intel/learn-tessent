// Sliding Diag (SlideDiag_DiagAll)
...
// entire array, writes the base diagonal to data, and increments the base diagonal. The SlideDiag_DiagAll is a variation of the SlideDiag in which all the operations in arrayDiags are optimized, i.e, the 2 diagonal writes and the block Read
// Short Notation:
// ^(wD1)
...
Algorithm (IntelLVSlideDiagFastxDiagAll){
...
//normal write to diag cell starting at odd column
  ...
  //(X+1, Y-1) 
  Instruction (INCREMENT_HOME_CELL_ADVANCE_Xp1Ym1) {
   OperationSelect : Nooperation;
   AddressSelectCmd : Select_A;
   X1AddressCmd : Increment;
  ...
  Instruction (READ_AWAY_ROW_ODD_1) {
   OperationSelect : ReadFastColumn;
   AddressSelectCmd : Select_B;
   Y1AddressCmd : Increment;
   CounterACmd: Increment;
  ...
  //move home cell to (X+1, Y+3) in next two instructions
  //(X+1, Y+1)
  Instruction (INCREMENT_HOME_CELL_ADVANCE_Xp1Yp1) {
   OperationSelect : Nooperation;
   AddressSelectCmd : Select_A;
   X1AddressCmd : Increment;
   Y1AddressCmd : Increment;
  ...
  //(X, Y+2)
  Instruction (INCREMENT_HOME_CELL_ADVANCE_XYp2) {
   OperationSelect : NoOperationFastColumn;
   AddressSelectCmd : Select_A;
   X1AddressCmd : Hold;
   Y1AddressCmd : Increment;
   NextConditions {
  ...
  Instruction (INCREMENT_AWAY_CELL_ADVANCE_COLUMN_ODD_1) {
   OperationSelect : Nooperation;
   AddressSelectCmd : Select_B;
   X1AddressCmd : Increment;
